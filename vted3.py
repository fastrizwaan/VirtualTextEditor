...
class IndexedFile:
    ...
    def _index_utf16(self, progress_callback=None):
        """UTF-16 line indexing supporting CR, LF, and CRLF correctly."""
        mm = self.mm
        total = len(mm)
        ...
        # Skip BOM if present
        start = 2 if total >= 2 else 0
        self.index = array('Q', [start])

        pos = start
        last_report = 0
        report_interval = 50_000_000
        ...
        while pos + 1 < total:
            ...
            if progress_callback and pos - last_report > report_interval:
                last_report = pos
                GLib.idle_add(progress_callback, pos / total)
            ...
            # Next break = nearest CR or LF
            candidates = [p for p in (crp, lfp) if p != -1]
            newline_pos = min(candidates)

            # Move past 2 bytes
            pos = newline_pos + 2
            self.index.append(pos)

            # Handle CRLF: skip LF after CR
            if crp != -1 and newline_pos == crp:
                if pos + 1 < total and mm[pos:pos+2] == LF:
                    pos += 2

        # Ensure EOF entry
        if not self.index or self.index[-1] != total:
            self.index.append(total)
    ...
    def __getitem__(self, line):
        ...
        # --- FIXED UTF-16 text decoding ---
        if self.encoding.startswith("utf-16"):
            text = raw.decode(self.encoding, errors="replace")

            # Remove exactly one newline (LF or CRLF), do NOT rstrip() blindly
            if text.endswith("\r\n"):
                return text[:-2]
            if text.endswith("\n"):
                return text[:-1]
...
class Renderer:
    ...
    def safe_get_cursor_pos(layout, byte_index):
        txt = layout.get_text() or ""
        max_b = len(txt.encode("utf-8"))
        b = max(0, min(byte_index, max_b))
        return layout.get_cursor_pos(b)
    ...
    def draw(self, cr, alloc, buf, scroll_line, scroll_x,
        ...
        #   Clamp byte-index
        # -------------------------------------------------------
        def safe_cursor_byte_for_text(text, byte_index):
            if not text:
                return 0
            max_b = len(text.encode("utf-8"))
            return max(0, min(byte_index, max_b))
        ...
        for ln in range(scroll_line, min(scroll_line + max_vis, total)):
            ...
            # BIDI paragraph info
            # ----------------------------------------------------------------
            base_dir, text_w, tlay = bidi_info(text)
            ...
            if sel_start_line <= ln <= sel_end_line:
                ...
                # start X
                if s_col <= len(text):
                    sb = visual_byte_index(text, s_col)
                    sb = safe_cursor_byte_for_text(text, sb)
                    spos, _ = safe_get_cursor_pos(tlay, sb)
                ...
                # end X
                if e_col <= len(text):
                    eb = visual_byte_index(text, e_col)
                    eb = safe_cursor_byte_for_text(text, eb)
                    epos, _ = safe_get_cursor_pos(tlay, eb)
            ...
            if text == "":
                ...
                tlay.set_text(text, -1)

            cr.set_source_rgb(*self.text_foreground_color)
            cr.move_to(base_x, y)
            PangoCairo.show_layout(cr, tlay)
        ...
        if getattr(buf, "preedit_string", None) and line_visible:
            ...
            byte_index = safe_cursor_byte_for_text(line_text, byte_index)
            spos, _ = safe_get_cursor_pos(pe_l, byte_index)
        ...
        if cursor_visible and line_visible:
            ...
            spos, wpos = safe_get_cursor_pos(cur_l, byte_index)
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def update_im_cursor_location(self):
        try:
            ...
            byte_index = min(byte_index, len((line_text or " ").encode("utf-8")))
