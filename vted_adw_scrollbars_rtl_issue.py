...
class Renderer:
    ...
    def compute_base_x(self, is_rtl, text_w, ln_w, scroll_x, view_w, doc_w):
        """
        Unified base-X computation used by draw(), cursor placement,
        hit-testing, IM cursor, and keep_cursor_visible().
        """
        ...
        # -----------------------------
        # RTL alignment (gedit-correct)
        # -----------------------------
        if doc_w <= available:
            # RTL text fits → stick to viewport right
            return ln_w + (available - text_w)

        # Document wider than viewport → align against doc right edge
...
class VirtualTextView(Gtk.DrawingArea):
    ...
    def on_resize(self, widget, width, height):
        ...
        self.hadj.set_page_increment(width // 2)
    ...
    def update_scrollbar(self):
        ...
        # GTK Adjustment: upper = total content units, NOT (total - visible)
        self.vadj.set_lower(0)
        ...
        # clamp scroll line inside range
...
class EditorWindow(Adw.ApplicationWindow):
    def __init__(self, app):
        ...
        self.hscroll.add_css_class("overlay-scrollbar")
