...
class Renderer:
    ...
    def draw(self, cr, alloc, buf, scroll_line, scroll_x,
        ...
        # --- helper: clamp byte index using the original text's UTF-8 length ---
        def safe_cursor_byte_for_text(text, byte_index):
            # text may be None or empty; use its UTF-8 length as the valid max
            max_b = len((text or "").encode("utf-8"))
            if byte_index < 0:
                return 0
            if byte_index > max_b:
                return max_b
