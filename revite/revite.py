...
# Optional: Import find/replace feature if available
try:
    from find_feature import install_find_feature, FindReplaceBar, get_css as get_find_css
    FIND_FEATURE_AVAILABLE = True
except ImportError:
...
# Optional: Import editing feature (Selection + InputController) if available
try:
    from editing_feature import (
        Selection,
        InputController,
        detect_rtl_line,
        install_editing_feature
    )
    EDITING_FEATURE_AVAILABLE = True
except ImportError:
    EDITING_FEATURE_AVAILABLE = False
    print("Note: Editing feature not available (editing_feature.py not found)")
    print("      Editor will have minimal functionality - install editing_feature.py for full features")
    
    # Minimal stub implementations to prevent crashes
    class Selection:
        def __init__(self):
            self.active = False
            self.start_line = self.start_col = self.end_line = self.end_col = 0
            self.selecting_with_keyboard = False
        def clear(self): self.active = False
        def set_start(self, line, col): pass
        def set_end(self, line, col): pass
        def has_selection(self): return False
        def get_bounds(self): return None, None, None, None
        def contains_position(self, line, col): return False
    ...
    class InputController:
        def __init__(self, view, buf):
            ...
            self.buf = buf
            self.dragging = False
        def click(self, ln, col): pass
        def start_drag(self, ln, col): pass
        def update_drag(self, ln, col): pass
        def end_drag(self): pass
        def move_left(self, extend_selection=False): pass
        def move_right(self, extend_selection=False): pass
        def move_up(self, extend_selection=False): pass
        def move_down(self, extend_selection=False): pass
        def move_word_left(self, extend_selection=False): pass
        def move_word_right(self, extend_selection=False): pass
        def move_home(self, extend_selection=False): pass
        def move_end(self, extend_selection=False): pass
        def move_document_start(self, extend_selection=False): pass
        def move_document_end(self, extend_selection=False): pass
    
    def detect_rtl_line(text):
        return False

# Optional: Import keyboard feature if available
try:
    from keyboard_feature import KeyboardHandler
    KEYBOARD_FEATURE_AVAILABLE = True
except ImportError:
    KEYBOARD_FEATURE_AVAILABLE = False
    print("Note: Keyboard feature not available (keyboard_feature.py not found)")
    class KeyboardHandler:
        def __init__(self, view, buf, ctrl): pass
        def on_key(self, c, keyval, keycode, state): return False

# Optional: Import clipboard feature if available
try:
    from clipboard_feature import ClipboardHandler
    CLIPBOARD_FEATURE_AVAILABLE = True
except ImportError:
    CLIPBOARD_FEATURE_AVAILABLE = False
    print("Note: Clipboard feature not available (clipboard_feature.py not found)")
    class ClipboardHandler:
        def __init__(self, view, buf): pass
        def copy_to_clipboard(self): pass
        def cut_to_clipboard(self): pass
        def paste_from_clipboard(self): pass

# Optional: Import mouse feature if available
try:
    from mouse_feature import MouseHandler
    MOUSE_FEATURE_AVAILABLE = True
except ImportError:
    MOUSE_FEATURE_AVAILABLE = False
    print("Note: Mouse feature not available (mouse_feature.py not found)")
    class MouseHandler:
        def __init__(self, view, buf, ctrl): pass
        def on_click_pressed(self, g, n, x, y): pass
        def on_click(self, ln, col): pass
        def on_click_released(self, g, n, x, y): pass
        def on_drag_begin(self, g, x, y): pass
        def on_drag_update(self, g, x, y): pass
        def on_drag_end(self, g, x, y): pass
        def on_middle_click(self, g, n, x, y): pass
        def on_right_click(self, g, n, x, y): pass
        ...
        def has_selection(self):
            return self.active and (self.start_line != self.end_line or self.start_col != self.end_col)
    ...
    class InputController:
        """Fallback InputController class"""
        def __init__(self, view, buf):
            self.view = view
            self.buf = buf
            self.dragging = False
        ...
        def move_down(self, extend_selection=False):
            ...
            if ln + 1 < b.total():
                ...
                if extend_selection:
                    last_line = b.get_line(ln)
                    b.set_cursor(ln, len(last_line), extend_selection)
        ...
        def move_word_right(self, extend_selection=False):
            ...
            while col < len(line) and line[col].isspace():
                col += 1
            if col == b.cursor_col and ln + 1 < b.total():
                ln += 1
                col = 0
            b.set_cursor(ln, col, extend_selection)
        
        def move_home(self, extend_selection=False):
            self.buf.set_cursor(self.buf.cursor_line, 0, extend_selection)
        ...
        def move_document_end(self, extend_selection=False):
            ...
            last_line = b.total() - 1
            if last_line >= 0:
                line = b.get_line(last_line)
                b.set_cursor(last_line, len(line), extend_selection)
...
# detect_rtl_line function is now imported from editing_feature.py
...
# Selection class is now imported from editing_feature.py
...
class VirtualBxuffer(GObject.Object):
    __gsignals__ = {
...
# InputController class is now imported from editing_feature.py
    ...
    def draw(self, cr, alloc, buf, scroll_line, scroll_x,
        ...
        for ln in range(scroll_line, total):
            ...
            for vis_idx in range(start_vis_idx, len(wrap_points)):
                ...
                # RTL detection for alignment
                rtl = line_is_rtl(text_segment)
                
                # Use minimal margin for RTL to maximize text space
                margin = 5 if rtl else self.right_margin_width
                text_area_width = max(100, alloc.width - ln_width - margin)
                
                # For RTL text, Pango alignment is relative to reading direction:
                # - ALIGN_LEFT = align to START of reading direction
                #   - For LTR: START = left edge (text appears on left)
                #   - For RTL: START = right edge (text appears on right)
                # So we always use ALIGN_LEFT for both - it means "align to start"
                layout.set_auto_dir(True)  # Let Pango detect RTL
                layout.set_width(int(text_area_width * Pango.SCALE))
                layout.set_wrap(Pango.WrapMode.WORD_CHAR)
                layout.set_alignment(Pango.Alignment.LEFT)  # Align to start of reading direction
                

                text_w, text_h = layout.get_pixel_size()

                
                # Position at left edge of text area
...
class VirtualTextView(Gtk.DrawingArea):
 ...
 # Shim for legacy external access
    ...
    def update_metrics(self):
        ...
        # Pass font to mapper for Pango-based RTL wrapping
        if hasattr(self.mapper, 'set_font'):
            self.mapper.set_font(self.font_desc)
        
        # Pass viewport width in pixels for accurate measurement
        viewport_width = self.get_allocated_width() or 800
        self.mapper.set_viewport_width(viewport_width, self.char_width)
        
        # Also update tab array for mapper
        if hasattr(self, 'tab_array') and hasattr(self.mapper, 'set_tab_array'):
            self.mapper.set_tab_array(self.tab_array)
    ...
    def update_colors_for_theme(self, is_dark=None):
        ...
        if hasattr(self.syntax, 'set_theme'):
    ...
    # ---- Renderer compatibility methods for editing_feature.py ----
    
    def calculate_line_number_width(self, cr, total_lines):
        """Calculate the width of the line number gutter."""
        if self.show_line_numbers:
            return max(30, int(len(str(total_lines)) * self.char_width) + 10)
        return 0
    
    def get_wrap_points_for_line(self, cr, buf, line_num, ln_width, alloc_width):
        """Get wrap points for a given line."""
        return self.mapper.get_line_segments(line_num)
    ...
    def calculate_text_base_x(self, is_rtl, text_w, alloc_w, ln_width, scroll_x):
        """Calculate base X position for text."""
        if is_rtl:
            return ln_width  # RTL aligns to left edge (Pango handles right-alignment)
        else:
            return ln_width + 2 - scroll_x
    ...
    def draw_view(self, area, cr, w, h):
        ...
        while visual_lines_drawn < visible_lines and current_log_line < total_lines:
            ...
            segments = self.mapper.get_line_segments(current_log_line, cr)
            ...
            for i, (seg_start, seg_end) in enumerate(segments):
                ...
                rtl = line_is_rtl(seg_text)
                
                # For RTL text, set up Pango to handle alignment properly
                # ALIGN_LEFT = align to START of reading direction
                # For RTL: START = right edge, so text appears on the right
                layout.set_auto_dir(True)
                layout.set_width(int(viewport_w * Pango.SCALE))
                layout.set_alignment(Pango.Alignment.LEFT)  # "Start" alignment works for both LTR and RTL

                # ---- text draw (single clip, correct) ----
                cr.save()
                # Use full viewport width for clipping to prevent cutoff
                # Extra padding on right to ensure RTL text isn't clipped
                cr.rectangle(ln_width, current_y, viewport_w + 20, self.line_h)
                cr.clip()
                
                # For RTL text, position at left edge of text area (no scroll offset)
                # For LTR text, apply normal scroll offset
                if rtl:
                    text_base_x = ln_width
                else:
                    text_base_x = ln_width + 2 - self.scroll_x
                    
                cr.move_to(text_base_x, current_y)
                ...
                if show_cursor and current_log_line == self.buf.cursor_line:
                    ...
                    if is_in_segment or is_at_end_of_last_segment:
                        # For cursor, use the layout with width set to get correct RTL position
                        layout.set_width(int(viewport_w * Pango.SCALE))
                        ...
                        # Use same base_x as text drawing for consistency
                        cx = text_base_x + pos.x / Pango.SCALE
                        
                        layout.set_width(-1)  # Reset
...
class VxirtualTextView(Gtk.DrawingArea):
    ...
    def __init__(self, buf):
        ...
        self.keyboard_handler = KeyboardHandler(self, buf, self.ctrl)
        self.clipboard_handler = ClipboardHandler(self, buf)
        self.mouse_handler = MouseHandler(self, buf, self.ctrl)
    ...
    def xpaste_from_clipboard(self):
        """Paste text from clipboard with better error handling and progress"""
        clipboard = self.get_clipboard()
...
# ============================================================
# FindReplaceBar is now in find_feature.py module
# It will be conditionally imported if available

# Install find feature methods into buffer and view classes
if FIND_FEATURE_AVAILABLE:
    install_find_feature(VirtualBuffer, VirtualTextView, None)
...
class EditorWindow(Adw.ApplicationWindow):
    ...
    def _create_editor_overlay(self, editor, add_close_button=False):
        ...
        # Create FindReplaceBar (if available)
        if FIND_FEATURE_AVAILABLE:
            editor.find_bar = FindReplaceBar(editor)
        else:
            editor.find_bar = None
